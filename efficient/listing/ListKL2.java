		...
		if(l == 1){
			...
			if(r==1){
				...
				while(vertices.hasNext()){
					...
					List<List<Graph.Vertex<Integer>>> kqList = find2(nx,l-1);
				...
				while(edges.hasNext()){
					...
					List<List<Graph.Vertex<Integer>>> kqList = find2(nXY,l-2);
					for(List<Graph.Vertex<Integer>> kq: kqList){
						Set<Integer> hh = new HashSet<Integer>(); //to store elements of the K(l-1) vertices
...
//				List<List<Graph.Vertex<Integer>>> kqList = find(graph,q);
				List<List<Graph.Vertex<Integer>>> kqList = find2(graph, q);
				...
				List<List<Graph.Vertex<Integer>>> k2qList = find2(graph,(2*q));
	...
	public List<List<Graph.Vertex<Integer>>> find2(UndirectedGraph<Integer,Integer> graph, int l){
		if(l<1)
			return null;
